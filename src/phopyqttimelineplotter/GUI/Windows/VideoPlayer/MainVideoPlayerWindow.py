    ...
    def _on_video_filename_changed(self):
        ...
        if not self.media.get_duration():
            ...
            if sys.platform.startswith("linux"):  # for Linux using the X Server
                ...
                self.media_player.set_nsobject(
                    int(self.ui.frame_video.mac_view.winId())
                )
            else:
                print(
...
class MainVideoPlayerWindow(
    ...
    def initUI(self):
        ...
        self.ui.spinBoxFrameJumpMultiplier.setValue(
            self.curr_frame_skip_jump_multiplier
        )
        self.ui.spinBoxFrameJumpMultiplier.valueChanged.connect(
            self.handle_jump_multiplier_value_changed
        )
    ...
    def get_media_total_num_frames(self):
        ...
        curr_total_duration = (
            float(curr_total_duration_millis) / 1000.0
        )  # Convert to seconds.
    ...
    def seek_frames(self, relativeFrameOffset):
        ...
        totalNumFrames = (
            self.get_media_total_num_frames()
        )  # This number is strangely multiplied by a factor of 1000.0 too! It's incorrect.
